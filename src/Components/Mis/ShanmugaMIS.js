...
// Col order: Date(0) | PatientID(1) | PatientName(2) | Barcode(3) | Age(4) | TestName(5)
// Approximate widths: 100 | 110 | 160 | 120 | 60 | 160
const STICKY_LEFT = [0, 100, 210, 370, 490, 550];
const STICKY_WIDTH = [100, 110, 160, 120, 60, 160];

const stickyTh = (colIndex) => `
  position: sticky;
  left: ${STICKY_LEFT[colIndex]}px;
  z-index: 3;
  background-color: #f9fafb;
  min-width: ${STICKY_WIDTH[colIndex]}px;
  max-width: ${STICKY_WIDTH[colIndex]}px;
  &::after {
    content: '';
    position: absolute;
    right: 0;
...
const stickyTd = (colIndex, bg = "white") => `
  position: sticky;
  left: ${STICKY_LEFT[colIndex]}px;
  z-index: 2;
  background-color: ${bg};
...
const ShanmugaMIS = () => {
  ...
  // Reset to page 1 whenever search or date changes
  useEffect(() => {
    setCurrentPage(1);
  ...
  // but keep patient groups intact (don't split a patient across pages).
  // Build page slices: accumulate groups until we hit ITEMS_PER_PAGE rows.
  const pages = [];
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const formattedFromDate = fromDateObj
        ...
        .replace(/\//g, "-");
      const formattedToDate = toDateObj
        .toLocaleDateString("en-GB")
        .replace(/\//g, "-");
                  ...
                  paginatedGroups.map((group, groupIndex) =>
                    group.tests.map((test, testIndex) => (
                      <tr key={`${groupIndex}-${testIndex}`}>
                        ...
                        {/* Test Name — always sticky-5, one row per test */}
