...
const TickIcon = () => (
  <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
    ...
    <path d="M3.5 6l1.8 1.8 3.2-3.6" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
...
const CHCApproval = ({ patient, onClose, onApprovalSaved }) => {
  ...
  const fetchPatientDetails = async () => {
    try {
      ...
      if (!result.success) {
        ...
        alert("Failed to fetch patient details: " + (result.error || "Unknown error"))
        return
      }
      ...
      if (invResult.success && invResult.data) {
        ...
        setPatientDetails((prev) => {
          ...
          // Vitals
          if ((!merged.vitals || Object.keys(merged.vitals).length === 0) && inv.vitals && Object.keys(inv.vitals).length > 0) {
            merged.vitals = inv.vitals
          }

          // Patient history
          if (!merged.medical_history && inv.patient_history && inv.patient_history.trim()) {
          ...
          if (!merged.ophthalmology) {
            const ophthalTest = (inv.chc_tests || []).find((t) => t.test_id === "CHCT001")
  ...
  const processUnicodeText = (text) => {
    if (!text) return ""
    const unicodeMap = { μ: "µ", α: "α", β: "β", γ: "γ", δ: "δ", Ω: "Ω", "²": "²", "³": "³", "⁴": "⁴", "°": "°", "±": "±", "×": "x", "÷": "/" }
  ...
  const LabInvestigationsPreview = ({ tests = [] }) => {
    ...
    if (labTests.length === 0) return <InfoValue>No lab investigations available.</InfoValue>
                    ...
                    <React.Fragment key={test.testname + (test.samplecollected_time || "")}>
                      <tr>
                        ...
                        <LabTd>
                          ...
                          {status === "H" && <StatusBadgeHl $high> ▲ H</StatusBadgeHl>}
                          {status === "L" && <StatusBadgeHl $low> ▼ L</StatusBadgeHl>}
                        </LabTd>
                        ...
                        (() => {
                          ...
                          return Object.keys(bySubtitle).map((subtitle) => (
                            <React.Fragment key={subtitle || "nosub"}>
                              {subtitle && (
                                <tr>
                                  <LabTd colSpan={6}><SubTitle>{subtitle}</SubTitle></LabTd>
                                </tr>
                              )}
                              {bySubtitle[subtitle].map((p) => {
                                ...
                                return (
                                  <tr key={p.name}>
                                    ...
                                    <LabTd>
                                      <span>{p.value || ""}</span>
                                      {pStatus === "H" && <StatusBadgeHl $high> ▲ H</StatusBadgeHl>}
                                      {pStatus === "L" && <StatusBadgeHl $low> ▼ L</StatusBadgeHl>}
                                    </LabTd>
        ...
        {isPDF ? (
          <>
            {images.length === 0 && !isConverting && (
              <button
                onClick={() => {
                  setConversionLoading((prev) => ({ ...prev, [fileKey]: true }))
                  convertPdfToImages(rawData, fileKey)
                }}
                style={{ width: "100%", padding: "12px", backgroundColor: "#DB9BB9", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 14, fontWeight: 600 }}
              >
                📄 Load PDF Preview
              </button>
            )}
            {isConverting && (
              <div style={{ padding: 20, textAlign: "center", background: "#f8f4f7", borderRadius: 6, color: "#888", fontSize: 14 }}>
                ⏳ Converting PDF to images…
              </div>
            )}
            {hasError && !isConverting && (
              <div style={{ padding: 16, background: "#fff5f5", border: "1px solid #feb2b2", borderRadius: 6, color: "#c53030", fontSize: 13 }}>
                ⚠️ Failed to render PDF.
                <button onClick={() => { setPdfImages((prev) => { const n={...prev}; delete n[fileKey]; return n }); setConversionLoading((prev) => ({ ...prev, [fileKey]: true })); convertPdfToImages(rawData, fileKey) }}
                  style={{ marginLeft: 10, padding: "4px 10px", cursor: "pointer", background: "#DB9BB9", color: "white", border: "none", borderRadius: 4 }}>
                  Retry
                </button>
              </div>
            )}
            {!hasError && images.length > 0 && !isConverting && images.map((img, idx) => (
              <div key={idx} style={{ marginBottom: 12 }}>
                {images.length > 1 && <div style={{ fontSize: 11, color: "#888", marginBottom: 4 }}>Page {idx + 1} / {images.length}</div>}
          ...
          rawData ? (
            <FileImage
              src={rawData.startsWith("data:") ? rawData : `data:${imgMime};base64,${rawData}`}
              alt={label}
            />
          ) : (
            <div style={{ color: "#888", fontSize: 13 }}>No file data available</div>
          )
        )}
      ...
      <FilePreviewContainer key={testId}>
        ...
        {entry.report?.trim() && (
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: "#4361ee", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Report
            </div>
        ...
        {entry.notes?.trim() && (
          <div style={{
            background: "#f6fff8", border: "1px solid #c6f6d5",
            borderRadius: 8, padding: "14px 16px", marginBottom: 14,
          }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#276749", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Impression / Notes
            </div>
            <div style={{ fontSize: 14, color: "#222", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
              {entry.notes}
        ...
        {/* ── File images / PDFs ─────────────────────────────────────── */}
        {entry.files && entry.files.map((file, fileIdx) => {
          if (!file) return null
          ...
          return (
            <div key={fileIdx} style={{ marginBottom: fileIdx < entry.files.length - 1 ? 20 : 0 }}>
              {entry.files.length > 1 && (
                <div style={{ fontSize: 12, color: "#888", marginBottom: 6, fontStyle: "italic" }}>
                ...
                <>
                  ...
                  {pdfImgList.length === 0 && !isConverting && (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: "#DB9BB9", color: "white",
                  ...
                  {isConverting && (
                    <div style={{
                      ...
                      background: "#f8f4f7", borderRadius: 6,
                  ...
                  {hasError && !isConverting && (
                    <div style={{
                      padding: 16, background: "#fff5f5",
                      border: "1px solid #feb2b2", borderRadius: 6,
                      color: "#c53030", fontSize: 13,
                    }}>
                      ⚠️ Failed to render PDF. Check console for details.
                      <button
                        onClick={() => {
                          setPdfImages((prev) => { const n = {...prev}; delete n[fileKey]; return n })
                          setConversionLoading((prev) => ({ ...prev, [fileKey]: true }))
                          convertPdfToImages(rawData, fileKey)
                        }}
                        style={{ marginLeft: 10, padding: "4px 10px", cursor: "pointer", background: "#DB9BB9", color: "white", border: "none", borderRadius: 4 }}
                    ...
                    <div>
                      {pdfImgList.map((img, pgIdx) => (
                        <div key={pgIdx} style={{ marginBottom: 12 }}>
                          ...
                          <FileImage src={img} alt={`${label || entry.label} page ${pgIdx + 1}`} />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                rawData ? (
                  <FileImage
                    ...
                    onError={(e) => {
                      ...
                      e.target.insertAdjacentHTML("afterend", "<div style='color:#c53030;font-size:13px'>⚠️ Image failed to load</div>")
                    }}
                  />
                ) : (
                  <div style={{ color: "#888", fontSize: 13 }}>No file data available</div>
            ...
            {loading ? (
              ...
              <>
                ...
                <ReportSection>
                  ...
                  <InfoRow><InfoLabel>Name:</InfoLabel><InfoValue>{patientDetails.patientname || "N/A"}</InfoValue></InfoRow>
                  <InfoRow><InfoLabel>Employee ID:</InfoLabel><InfoValue>{patientDetails.patient_id || "N/A"}</InfoValue></InfoRow>
                  <InfoRow><InfoLabel>Age / Gender:</InfoLabel><InfoValue>{patientDetails.age} / {patientDetails.gender}</InfoValue></InfoRow>
                  <InfoRow><InfoLabel>Date:</InfoLabel><InfoValue>{formatDate(patientDetails.date)}</InfoValue></InfoRow>
                  <InfoRow><InfoLabel>Barcode:</InfoLabel><InfoValue>{patientDetails.barcode}</InfoValue></InfoRow>
                  {patientDetails.company_name && (
                    <InfoRow><InfoLabel>Company:</InfoLabel><InfoValue>{patientDetails.company_name}</InfoValue></InfoRow>
                  )}
                ...
                {patientDetails.investigation_notes && Object.keys(patientDetails.investigation_notes).length > 0 && (
                  <ReportSection>
                    ...
                    {patientDetails.investigation_notes.ecg_notes && (
                      <InfoRow><InfoLabel>E.C.G:</InfoLabel><InfoValue>{patientDetails.investigation_notes.ecg_notes}</InfoValue></InfoRow>
                    )}
                    {patientDetails.investigation_notes.pft_notes && (
                      <InfoRow><InfoLabel>Spirometry:</InfoLabel><InfoValue>{patientDetails.investigation_notes.pft_notes}</InfoValue></InfoRow>
                    )}
                    {patientDetails.investigation_notes.xray_notes && (
                      <InfoRow><InfoLabel>X-Ray:</InfoLabel><InfoValue>{patientDetails.investigation_notes.xray_notes}</InfoValue></InfoRow>
                    )}
                    {patientDetails.investigation_notes.audiometry_notes && (
                      <InfoRow><InfoLabel>Audiometry:</InfoLabel><InfoValue>{patientDetails.investigation_notes.audiometry_notes}</InfoValue></InfoRow>
                    )}
                  </ReportSection>
                )}
                ...
                {(Object.keys(chcInvestigationFiles).length > 0 || Object.keys(investigationFiles).length > 0) && (
                  <ReportSection>
                    ...
                    {Object.keys(chcInvestigationFiles).length > 0 ? (
                      Object.entries(chcInvestigationFiles).map(([testId, entry]) =>
                ...
                {(() => {
                  ...
                  const isCHC = oph.distance || oph.nearVision || oph.colourVision

                  const rows = []
                  if (isCHC) {
                    if (oph.distance)        rows.push({ label: "Distant Vision",  r: oph.distance.right,     l: oph.distance.left })
                    if (oph.nearVision)      rows.push({ label: "Near Vision",     r: oph.nearVision.right,   l: oph.nearVision.left })
                    if (oph.colourVision)    rows.push({ label: "Colour Vision",   r: oph.colourVision.right, l: oph.colourVision.left })
                    if (oph.ocularmovement)  rows.push({ label: "Ocular Movement", r: oph.ocularmovement.right, l: oph.ocularmovement.left })
                  } else if (oph.visual_acuity) {
                    const va = oph.visual_acuity
                    if (va.distance)      rows.push({ label: "Distant Vision",  r: va.distance.right,      l: va.distance.left })
                    if (va.near_vision)   rows.push({ label: "Near Vision",     r: va.near_vision.right,   l: va.near_vision.left })
                    if (va.color_vision)  rows.push({ label: "Colour Vision",   r: va.color_vision.right,  l: va.color_vision.left })
                    if (va.ocularmovement) rows.push({ label: "Ocular Movement", r: va.ocularmovement.right, l: va.ocularmovement.left })
                  }

                  const complaints = oph.complaints || oph.patient_complaints || ""
                  ...
                  return (
                    <ReportSection>
                      ...
                      {rows.length > 0 && (
                        <OphthalmologyTable>
                          ...
                          <tbody>
                            {rows.map((row) => (
                              <tr key={row.label}>
                                <OphthalmologyTd style={{ fontWeight: 600 }}>{row.label}</OphthalmologyTd>
                                <OphthalmologyTd>{row.r || "N/A"}</OphthalmologyTd>
                                <OphthalmologyTd>{row.l || "N/A"}</OphthalmologyTd>
                              </tr>
                            ))}
                          </tbody>
                        </OphthalmologyTable>
                      ...
                      <div style={{ marginTop: "15px" }}>
                        ...
                        <InfoValue>
                          {remarks?.trim() || "Both Eyes: Normal Vision. Review after 6 months or 1 year."}
                        </InfoValue>
                      </div>
                ...
                <ReportSection>
                  ...
                  <InputGroup>
                    ...
                    <TextArea value={impression} onChange={(e) => setImpression(e.target.value)} placeholder="Enter impression..." />
                  </InputGroup>
                  <InputGroup>
          ...
          <>
            <StatusList>
              ...
              {chcTests.length > 0 ? (
                ...
                <StatusItem>
                  <StatusInfo>
                    <StatusLabel style={{ color: "#999", fontStyle: "italic" }}>No CHC tests found</StatusLabel>
                  </StatusInfo>
                </StatusItem>
              )}
