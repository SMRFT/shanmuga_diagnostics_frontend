// PDF generation for the selected/sorted tests of a patient.
// Extracted verbatim from the `handlePrint` function that used to live
// inside TestSorting — logic is unchanged, only relocated.
...
// `tests` is the full list of tests for this patient (used only for the
// "Result/s to follow" unapproved-tests comparison at the end); `selectedTests`
// is the subset the user picked to print/share.
export default async function generateSortedTestsPdf(
  selectedTests,
  tests,
  patient,
  withLetterpad,
  ...
  try {
    ...
    const unapprovedTests = allTestStatuses.filter((hmsTest) => {
      return !tests.some((sortingTest) => sortingTest.test_id === hmsTest.test_id);
    });
