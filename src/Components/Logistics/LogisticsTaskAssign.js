...
// ============ Table Styles ============
const TableCard = styled(FormCard)`
  margin-top: 2rem;
  overflow: hidden;
`;
...
const BuildingIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M3 21h18M9 8h1m-1 4h1m-1 4h1M15 8h1m-1 4h1m-1 4h1M5 21V5a2 2 0 012-2h10a2 2 0 012 2v16" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const TableIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M3 3h18v18H3V3zm0 6h18M9 3v18" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const LogisticsTaskAssign = () => {
  ...
  useEffect(() => {
    if (formData.clinicalname) {
      fetchSalesMapping(formData.clinicalname);
    } else {
  ...
  const fetchClinicalNames = async () => {
    try {
      ...
      showToast('Failed to load lab names. Please try again.', 'error');
      setClinicalNames([]);
    } finally {
  ...
  const fetchSampleCollectors = async () => {
    try {
      ...
      console.log('Sample Collectors API Response:', response);
      ...
      if (response?.data?.collectors) {
        ...
        if (Array.isArray(response.data.collectors)) {
          collectors = response.data.collectors;
        } 
        // If it's an object, convert values to array
        else if (typeof response.data.collectors === 'object') {
          collectors = Object.values(response.data.collectors);
        }
      } 
      ...
      // If response has 'results' property
        ...
        collectors = response.results;
      }

      // Filter out null, undefined, empty strings, and ensure each is a string
      ...
      showToast('Failed to load sample collectors. Please try again.', 'error');
      setSampleCollectors([]);
    } finally {
  ...
  const fetchTasks = async (date) => {
    try {
      ...
      const response = await apiRequest(
        `${Labbaseurl}logistics/?date=${date}`,
        'GET'
      );
  ...
  const fetchSalesMapping = async (clinicalName) => {
    try {
      const clinic = clinicalNames.find(
        item => item.clinicalname === clinicalName
      );
      
      if (clinic && clinic.salesMapping) {
        setFormData(prev => ({
          ...prev,
          sales_person: clinic.salesMapping,
        }));
      } else {
        ...
        if (Array.isArray(data) && data.length > 0) {
          setFormData(prev => ({
            ...prev,
            sales_person: data[0].salesMapping || '',
          }));
        } else if (data && data.salesMapping) {
          setFormData(prev => ({
            ...prev,
            sales_person: data.salesMapping || '',
          }));
        } else {
          setFormData(prev => ({ ...prev, sales_person: '' }));
        }
  ...
  const validateForm = () => {
    if (!formData.clinicalname.trim()) {
      showToast('Please select a lab name', 'error');
      return false;
    }
    if (!formData.sample_collector.trim()) {
      showToast('Please select a sample collector', 'error');
      return false;
    }
    if (!formData.sales_person.trim()) {
      showToast('Sales person is required', 'error');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(err.message || 'Failed to assign task. Please try again.', 'error');
      console.error('Error creating logistics:', err);
    } finally {
      setLoading(false);
    }
  };
            ...
            <FormGrid>
              ...
              <FormGroup>
                ...
                <InputWrapper>
                  ...
                  <Input
                    ...
                    disabled
                    placeholder="Auto-filled from Lab Name"
                    hasIcon
                  />
                </InputWrapper>
              </FormGroup>
              ...
              <FormGroup>
                ...
                <InputWrapper>
                  ...
                  >
                    <option value="">
                      {fetchingCollectors ? 'Loading collectors...' : 'Select Collector'}
                    </option>
                    {sampleCollectors.map((collector, index) => (
        ...
        <TableCard>
          <Header>
            <HeaderContent>
              ...
              <Subtitle>View and manage daily sample collection tasks</Subtitle>
            </HeaderContent>
          </Header>
          ...
          <TableHeader>
            ...
            <DateFilterWrapper>
              <DateLabel>Filter by Date:</DateLabel>
              <DateInput
                type="date"
                value={selectedDate}
