...
const TestSorting = ({ patient, onClose }) => {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      ...
      try {
        let apiDate = patient.date || "";
        if (typeof apiDate === "string" && apiDate.includes("-")) {
        ...
        const response = await apiRequest(
          `${Labbaseurl}patient_test_sorting/?barcode=${patient.barcode}&date=${apiDate}`,
          "GET",
          null,
          {},
          {},
  ...
  const handlePrint = async (withLetterpad) => {
    ...
    try {
      ...
      const leftDetails = [
        ...
        { label: "Order ID", value: patientDetails.order_id || patient?.order_id || "N/A" },
      ];
