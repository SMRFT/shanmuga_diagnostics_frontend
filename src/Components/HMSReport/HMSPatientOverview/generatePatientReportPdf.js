// PDF report generation for a single HMS patient's approved test details.
// Extracted verbatim from the `handlePrint` function that used to live
// inside HMSPatientOverview — logic is unchanged, only relocated.
...
// `deps` carries the small bits of component state/config the original
// closure captured directly: the lab API base URL and the setLoading
// state setter (only ever used here to flip loading back to false).
export default async function generatePatientReportPdf(
  patient,
  withLetterpad = true,
  { Labbaseurl, setLoading },
) {
  try {
