...
const FranchiseSampleUpdate = () => {
    ...
    const fetchSampleCollected = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          setError(response.error || "Failed to fetch samples");
        }
      } catch (err) {
  ...
  const updateTestStatus = async (patientId, testIndex) => {
    ...
    try {
      // Changed to use barcode instead of patient_id in the API endpoint
      const response = await apiRequest(
        `${Labbaseurl}update_franchise_sample/${selectedPatient.barcode}/`,
        "PUT",
        {
          updates: [
            {
